...
public class Banner {
    ...
    public static Banner of(MultipartFile image, Theme theme) throws IOException {
        ...
        return Banner.builder().idx(idx).title(image.getOriginalFilename()).path(photoImg).size(image.getSize()).contentType(image.getContentType()).theme(theme).build();
    }
//
//    public BannerDto toBannerDto() {
//        return BannerDto.builder().title(this.title).path(this.path).contentType(this.contentType).build();
//    }
